...
```{r}
## test for top n fold change protein

# se ho bisogno del top n assoluto:
vect <- r6$stat_table %>% 
  dplyr::slice_max(abs(rev_vs_dmso_fold_change), n = 20) %>% 
  dplyr::pull(gene_names)

# se ho bisogno del top n sia up che down

vec_up <- r6$stat_table %>% 
  dplyr::slice_max(rev_vs_dmso_fold_change, n = 10) %>% 
  dplyr::pull(gene_names)

vec_down <- r6$stat_table %>% 
  dplyr::slice_min(rev_vs_dmso_fold_change, n = 10) %>% 
  dplyr::pull(gene_names)

vect <- c(vec_up, vec_down)
```
...
```{r plot-volcano}
r6$plot_volcano(test = tests, highlights_names = vect)
```
