...
pre_process <- function(data, pattern_interest){
  preprocess <- data %>% 
    ...
    dplyr::rename(unique_gene_names = gene_names) %>% ## every protein groups now have only 1 gene name associated to it
...
standardized_data <- pre_process(data = raw_data, pattern_interest="lfq_intensity_bc_" )
```
...
ggplot(p,aes(x= label, y= counts, fill=condition))+
  geom_bar(stat="identity", width = width_boxplot, color="black")+
...
plotx <- function(data, width_boxplot = 0.6)
  
  plotx(data = data_filtered, width_boxplot = 0.9)
