...
#                          rev = TRUE, cont = TRUE, oibs = TRUE, rescue_cont = NULL) 
      ...
      # data <- self$data 
        ...
        data_wrang <- data %>% 
          ...
          ## filter on peptides:
          {if(pep_filter == "peptides"){dplyr::filter(., peptides >= 2)}
            else if (pep_filter == "unique") {dplyr::filter(., unique_peptides >= 2)}
            else {dplyr::filter(., razor_unique_peptides >= 2)}}
...
ggplot()+
  geom_bar(mapping= aes(x= condition, y= counts, fill=color_palette))+
             theme(plot.title = element_text(hjust = 0.5))+
             labs(title= "Protein counts by condition")+ theme(legend.position = 'bottom')
