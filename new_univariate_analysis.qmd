...
```{r}
intact <- prova %>% 
  rba_reactome_interactors_psicquic(proteins=prova$SYMBOL, resource="IntAct", details = TRUE)
...
```{r}
Biogrid <- prova %>% 
  rba_reactome_interactors_psicquic(proteins=prova$SYMBOL, resource="BioGrid", details = TRUE)
...
source_biogrid <- source_db (biogrid_df)

edges_db <- function(data){
  data <- data %>% 
  dplyr::pull(Target) %>%
  bitr(fromType="UNIPROT", toType="SYMBOL", OrgDb="org.Hs.eg.db") %>% 
  dplyr::rename("Target"="SYMBOL") %>%
...
edges_biogrid <- edges_db (biogrid_df)
