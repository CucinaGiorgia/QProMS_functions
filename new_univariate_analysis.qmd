...
source(file = "C:/Users/MSU/Documents/GC/Analysis/QProMS_test_functions/myfun_test.R")
source(file = "C:/Users/MSU/Documents/GC/Analysis/QProMS_test_functions/theme_ggplot_test.R")
...
#PLOT Attempt
  p <- echarts4r::e_charts(animation=FALSE) %>% 
    echarts4r::e_graph(roam=TRUE, 
                       force= list( initLayout = "force", 
                                    repulsion=100,
                                    edgeLength=30,
                                    layoutAnimation=FALSE),
                       itemStyle=list(opacity=0.65),
                       lineStyle=list(curveness=0.2, color=""), 
                       emphasis=list(focus="adjacency", 
                                     lineStyle=list(width=10))) %>% 
    echarts4r::e_graph_nodes(nodes=nodes1, names = name,  value=value, size=size, category=grp) %>% 
    echarts4r::e_graph_edges(edges=edges, source=Source, target=Target, value=Value_i, size = Value_i) %>%
    echarts4r::e_color(c("blue", "red")) %>% 
    echarts4r::e_labels(nodes1$name, font_size=4)%>%
...
intact <- nodes %>% 
  pull(name) %>% 
  rba_reactome_interactors_psicquic(resource="IntAct", details = TRUE)
...
#Trasformo Source in SYMBOL
source_intact <- source_db(data = intact_df)

#Trasformo Target in SYMBOL
edges_intact <- function(data){
  data <- data %>% 
  dplyr::pull(Target) %>%
  bitr(fromType="UNIPROT", toType="SYMBOL", OrgDb="org.Hs.eg.db") %>% 
...
edges1 <- source_intact %>% 
  dplyr::pull(Target) %>%
  bitr(fromType="UNIPROT", toType="SYMBOL", OrgDb="org.Hs.eg.db") %>% 
  dplyr::rename("Target"="SYMBOL") %>%
  dplyr::left_join(source_intact, by= c("UNIPROT"="Target")) %>% 
  dplyr::rename("Value"="Score") %>%
  dplyr::mutate(loop=if_else(Source==Target, 0, 1)) %>% 
  dplyr::filter(loop!=0) %>% 
  dplyr::select(Source, Target, Value) %>% 
...
nodes_true <- edges1 %>% 
  dplyr::left_join(nodes0, by = "column") %>%
  dplyr::mutate(node1=if_else(Target==name, 0, 1))  %>%
  dplyr::mutate(node2=if_else(Source==name, 0, 1)) %>% 
  dplyr::filter(node=="0") %>% 
  dplyr::select(Source, Target, Value) %>% 
  distinct(Value, .keep_all = TRUE) 
...
intact_edge <- function(gene_vector){
  ...
  intact_list <-
    rba_reactome_interactors_psicquic(gene_vector, resource = "IntAct", details = TRUE) 
  
  all_interactions <-
    purrr::map(.x = 1:length(gene_vector),
               .f = ~ intact_list$entities[[.x]])
  
  res <-
    purrr::map(.x = all_interactions, .f = ~ keep_only_if_present(.x)) %>%
    purrr::compact() %>%
...
library(OmnipathR)
 ...
 complexes_query_genes_any <- unique(get_complex_genes(complexes,query,
    total_match=FALSE))
...
edges_corum <-  complexes_query_genes_any %>%
   ...
   dplyr::rename(Source=target, Target=nodo) %>% 
   dplyr::select(Source, Target, Value)
 ...
 echarts4r::e_charts(animation=FALSE) %>% 
    echarts4r::e_graph(roam=TRUE, 
                       ...
                       emphasis=list(focus="adjacency", 
                                     lineStyle=list(width=10))) %>% 
    echarts4r::e_graph_nodes(nodes=nodes, names = name,  value=value, size=size, category=grp) %>% 
    echarts4r::e_graph_edges(edges=edges_corum, source=Source, target=Target, value=Value, size = Value) %>%
    echarts4r::e_color(c("blue", "red")) %>% 
 ...
 edges_all <- edges %>% 
  dplyr::left_join(edges_corum, by = "Source") %>% 
  dplyr::select(Source, Target.x, Value.x, Value.y) %>% 
   dplyr::rename(Target=Target.x, Value_s=Value.x, Value_i=Value.y) %>% 
  dplyr::select(Source, Target, Value_s, Value_i) %>% 
  dplyr::mutate(Value_corum=(dplyr::if_else(is.na(Value_i), 0, Value_i))) %>%
  dplyr::select(Source, Target, Value_s, Value_corum) %>% View()
  ...
  echarts4r::e_charts(animation=FALSE) %>% 
    echarts4r::e_graph(roam=TRUE, 
                       ...
                       emphasis=list(focus="adjacency", 
                                     lineStyle=list(width=10))) %>% 
    echarts4r::e_graph_nodes(nodes=nodes, names = name,  value=value, size=size, category=grp) %>% 
    echarts4r::e_graph_edges(edges=edges_all, source=Source, target=Target, value=Value, size = Value) %>%
    echarts4r::e_color(c("blue", "red")) %>% 
    echarts4r::e_labels(nodes$name, font_size=4)%>%
    echarts4r:: e_title("Network", "Up & Down Regulated") %>%
    echarts4r::e_tooltip()
