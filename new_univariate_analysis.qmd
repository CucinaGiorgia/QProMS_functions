...
Edges <- string_db$get_interactions(Nodes$STRING_id) %>%
  ...
  inner_join(., Test, by=c('from'='STRING_id')) %>% 
  inner_join(., Test, by=c('to'='STRING_id')) %>% 
...
g <- graph_from_data_frame(Edges, directed = FALSE, vertices = Test$SYMBOL) %>% 
...
00000# Tentativo con Network
net <- network(all_edges,
        vertex.attr = Test,
        directed = FALSE,
        ignore.eval=FALSE)
