...
#Solo nodi CORUM (NON FUNZIONA)
nodes_corum <- function(data){
  data <- data %>% 
...
nodes_corum <- nodes_corum(data = my_complexes)
```

```{r STRING+CORUM}
#Nodi solo di CORUM
 nodes_corum <- my_complexes %>%
   dplyr::select(components_genesymbols) %>% 
   tidyr::separate_rows(components_genesymbols, sep = "_") %>%
   dplyr::filter(components_genesymbols %in% query) %>%
   dplyr::distinct(components_genesymbols, .keep_all = TRUE) %>% #Tolgo nomi duplicati 
...
final_edges_corum <- color_complex %>% 
  dplyr::right_join(edges_corum_complexes)
