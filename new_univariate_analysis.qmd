...
library(OmnipathR)
 ...
 complexes_query_genes_any %>%  tidyr::separate_rows(components_genesymbols, sep = "_") %>%
   dplyr::filter(components_genesymbols %in% query) %>%  
   janitor::get_dupes(name) %>%
   dplyr::select(name, dupe_count, components_genesymbols) %>% 
   dplyr::filter(dupe_count == 2) %>%
   dplyr::group_by(name) %>% 
   dplyr::group_map() %>% 
   dplyr::mutate(target = components_genesymbols) %>% 
   tidyr::crossing(components_genesymbols, target, .name_repair = "minimal" ) %>% 
   View()
   dplyr::mutate(unique_names = paste0( name, "__", components_genesymbols)) %>% 
   dplyr::distinct(unique_names, .keep_all = TRUE) %>% 
   dplyr::select(unique_names, name, components_genesymbols) %>% 
   dplyr::group_by(name) %>% 
   tidyr::separate_longer_delim(unique_names, delim = "__") %>% 
