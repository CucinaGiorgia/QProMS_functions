...
```{r Significant Up}
sig_up <- function(data, expression){
  data <- data %>% 
  dplyr::filter(., !regulation == expression)
}

up <- sig_up(data=sig, expression="Down")
```
...
Nodes_api <- rba_string_map_ids(ids = sig_string1, species = 9606, echo_query = TRUE) %>%
...
Edges_api <- rba_string_interactions_network(ids = Nodes_api %>% pull(stringId), species = 9606) %>% 
  ...
  dplyr::mutate(escore = escore*10)


Nodes_api_df <- Nodes_api %>%
  dplyr::rename(name=preferredName) %>% 
  dplyr::left_join(sig, by= c("name"="gene_names")) %>% 
  dplyr::select(-c(stringId, annotation, rev_vs_dmso_fold_change)) %>% 
  dplyr::rename(value =rev_vs_dmso_p_adj, size = rev_vs_dmso_p_val, grp = regulation) %>% 
  dplyr::mutate(value = log10(value)) %>% 
  dplyr::mutate(size = log10(size)^2) %>% 
...
set.seed(123)
echarts4r::e_charts() %>% 
  e_graph(roam=TRUE, itemStyle=list(opacity=0.65),layoutAnimation=FALSE ) %>% 
  e_graph_nodes(nodes=Nodes_api_df, names = name,  value=value, size=size, category=grp) %>% 
  e_graph_edges(edges=Edges_api_df, source=Source, target=Target, value=Value, size = Value) %>%
  e_color(c("blue", "red")) %>% 
  e_title("Network", "Up & Down Regulated") %>%
...
echarts4r::e_charts() %>% 
  e_graph_gl(itemStyle=list(opacity=0.65)) %>% 
  e_graph_nodes(nodes=Nodes_api_df, names = name,  value=value, size=size, category=grp) %>% 
  e_graph_edges(edges=Edges_api_df, source=Source, target=Target, value=Value, size=Value) %>%
  e_color(c("blue", "red")) %>%
  e_title("Network", "Up & Down Regulated") %>%
...
```{r Up}
up_nodes <- up %>% dplyr::pull(gene_names) %>% 
  ...
  dplyr::rename(value =rev_vs_dmso_p_adj, size = rev_vs_dmso_p_val, grp = regulation) %>%
  dplyr::mutate(value = log10(value)) %>% 
  dplyr::mutate(size = log10(size)*10) %>% 
  dplyr::mutate(grp = as.factor(grp))
  
up_edges <-up_nodes %>% dplyr::pull(name) %>%
  rba_string_interactions_network(species = 9606) %>%  
  dplyr::mutate(Names= paste0(preferredName_A, "_", preferredName_B)) %>% 
  dplyr::distinct(Names, .keep_all = TRUE) %>% 
  dplyr::rename(Source = preferredName_A, Target=preferredName_B, Value=escore) %>% 
  dplyr::select(Source,Target, Value) %>%
  dplyr::filter(Value>0.9) %>% #FILTRO SU ESCORE
  dplyr::mutate(Value = Value*10)

# Filter nodes degree 0
degree_filtered <- up_nodes %>% 
  left_join(up_edges, by=c("name"="Source")) %>% 
  left_join(up_edges, by=c("name"="Target")) %>% 
...
echarts4r::e_charts(animation=FALSE) %>% 
  e_graph(layout ="circular", itemStyle=list(opacity=0.65) ) %>% 
