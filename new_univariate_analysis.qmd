...
sig <- significant(data=r6$stat_table, test = "nw_vs_dmso")
```
...
edges_corum <-  complexes_query_genes_any %>%
  ...
  dplyr::mutate(names = paste0(Source, "__", Target)) %>%
...
#    dplyr::mutate(color= if_else(Value==10, "red","green")) %>% 
#   dplyr::mutate(Value= if_else(color=="red", 3, Value)) %>% 
#   as.data.frame() %>% 
#   dplyr::mutate(Value = round(Value, 0)) #Unisco CORUM a STRING
 
#Nodi solo di CORUM
 nodes_corum <- edges_corum %>%
   dplyr::mutate(name=paste0(Source, "_", Target)) %>%  
   dplyr::select(name, Value) %>% 
   dplyr::rename(value=Value) %>% 
   tidyr::separate_rows(name, sep = "_") %>%
   dplyr::filter(name %in% query) %>% 
   dplyr::distinct(name, .keep_all = TRUE) %>% 
   dplyr::mutate(grp="corum") %>% 
   dplyr::left_join(nodes %>% dplyr::select(name, size),by = "name")

#Tutti i nodi da rappresentare 
 nodes_all <- nodes %>% 
   dplyr::mutate(grp="string") %>% 
   dplyr::bind_rows(nodes_corum) %>% 
   dplyr::distinct(name, .keep_all = TRUE)
...
p <-echarts4r::e_charts(animation = FALSE) %>%
   ...
   echarts4r::e_tooltip()
p
#NETWORK CORUM+STRING (only 1-1 interactions)
...
```{r STRING+CORUM}
 # edges_all <-  edges %>% dplyr::bind_rows(edges_corum) %>% 
 #   dplyr::mutate(color= if_else(Value==10, "red","green")) %>% 
 #  as.data.frame() #Unisco CORUM a STRING
 # 
 
#Nodi solo di CORUM
 nodes_corum <- complexes_query_genes_any %>%
...
edges_all <- edges_all %>%
  dplyr::mutate(Value = round(Value, 0)) 
      # dplyr::filter(Source == "HNRNPL", Target == "HNRNPU", Value == 10) %>%
      # pull(color)
...
edges_corum_complexes <- complexes_query_genes_any %>%
  dplyr::select(name, components_genesymbols) %>%
  tidyr::separate_rows(components_genesymbols, sep = "_") %>%
  dplyr::filter(components_genesymbols %in% query) %>%
...
n_of_color <- edges_corum_complexes %>% dplyr::group_by(name) %>% dplyr::group_keys() %>% nrow()
...
color_complex <- edges_corum_complexes %>% 
  dplyr::group_map(~.x) %>% 
  purrr::map(
  .x = 1:rowcolorpalette,
  .f =  ~ dplyr::mutate(edges_corum_complexes[[.x]], color = color_palette[.x])
) %>%
  purrr::reduce(bind_rows)
...
final_edges_corum <- color_complex %>%  
  dplyr::right_join(edges_corum, by="Source") %>% 
  dplyr::select(Source, Target, Value, name, color) 
...
echarts4r::e_charts(animation=FALSE) %>% 
    echarts4r::e_graph(roam=TRUE, 
                       ...
                       autoCurveness = TRUE,
                       emphasis=list(focus="adjacency")) %>% 
    echarts4r::e_graph_nodes(nodes=nodes, names = name,  value=value, size=size) %>% 
