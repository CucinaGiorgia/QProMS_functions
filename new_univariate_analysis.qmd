...
deg <- degree(g, mode = "all") #se voglio che al dimension edei nodi corrisponda al grado
...
plot(net, vertex.cex=3, network.layout.fruchtermanreingold(net))



#Tentativo con VisNetwork
visNetwork(Test, all_edges) %>% 
  visIgraphLayout() %>% 
  visNodes(size = 10)
...
boh <- renderSimpleNetwork({
  simpleNetwork(all_edges,
                   fontSize = 15,
                   linkColour ="#013848",
                   zoom = TRUE)
})

forceNetwork(Links = all_edges$FROM, Nodes = Test$SYMBOL, NodeID = "SYMBOL", Group = "group")
...
ui <- shinyUI(fluidPage(
  ...
  sidebarPanel(
    sliderInput("opacity", min = 0.1, max = 1, value = 0.5, step = 0.1, label = "Opacity")
  ),
  mainPanel(
    tabPanel("Simple Network", simpleNetworkOutput("simple"))
   ),
  textInput("text", value = "", label = "Search Proteins")
  ))
