  ...
  for (i in 1:n_nodes) {
    ...
    list <-
      purrr::modify_in(list,
                       list("x", "opts", "series", 1, "data", i),
                       \(x) c(x, c(itemStyle= list(borderWidth = 2,
                           borderColor = if_else(category == "Up", "red", "blue")
                         ))
                       ))
  }
  return(list)
...
make_edges <- function(source, names) {
      ...
      if("corum" %in% source) {
        ...
        if (nrow(raw_corum_table) != 0) {
          ...
          final_table <- edges_corum_table %>%
            dplyr::mutate(comb_name = paste0(target, "_", source)) %>%
            group_by(comb_name) %>%
            dplyr::left_join(edges_corum_table) %>%
            dplyr::summarize(complex = paste(complex, collapse = ";"),
                             score = sum(score)) %>%
            tidyr::separate_wider_delim(cols = comb_name,
                                        delim = "_",
                                        names = c("source", "target")) %>%
            dplyr::mutate(color = "#4daf4a")
...
```{r}
library(purrr)
nodes <- make_nodes(list_from = "univariate", focus = "tc1d22b_vs_ev", direction = "both")

nodes_name <- nodes %>% dplyr::pull(gene_names)
edges <- make_edges(source = c("corum", "string"), names=nodes_name )

p <- plot_ppi_network(edge = edges, node = nodes, list_from = "univariate", score_thr = 0.8, isolate_nodes = FALSE, layout = "force", show_names = FALSE)
p

edges_comb_complex <- edges_corum_table %>% 
  dplyr::mutate(comb_name=paste0(target, "_", source)) %>%
  group_by(comb_name) %>% 
  dplyr::left_join(edges_corum_table) %>%
  dplyr::summarize(complex=paste(complex, collapse=";"),
                   score=sum(score)) %>%
  tidyr::separate_wider_delim(cols = comb_name, delim = "_", names = c( "source", "target")) %>% 
  dplyr::mutate(color = "#4daf4a") %>% 
  View()
```
...
raw_data_final <- raw_data %>% 
  ...
  dplyr::mutate(protein_i_ds= stringr::str_extract(protein_i_ds, "[^;]*")) %>% View()
  pull(protein_i_ds) 
...
uniprot_information <-
  fetch_uniprot(
    uniprot_ids = raw_data_final,
    columns = c("xref_pdb")
  ) %>%  
tidyr::drop_na("xref_pdb") %>%
  dplyr::mutate(pdb_id = stringr::str_extract(xref_pdb, "[^;]*")) %>% View()
...
prova <- 
  r3dmol() %>%
  m_add_model(data = m_fetch_pdb("2HF6"), format = "pdb") %>%
  m_set_style(style = c(m_style_cartoon(), m_style_stick())) %>%
  m_zoom_to() %>% 
    # Set style of structures
    m_set_style(style = m_style_cartoon(color = "#00AFBB")) %>%
    # Style the alpha helix
  m_set_style(
    sel = m_sel(ss = "h"), # Style alpha helix
    style = m_style_cartoon(color = "#636efa")
  ) %>% 
    m_set_style(
    sel = m_sel(ss = "s"), # Style beta sheet
    style = m_style_cartoon(color = "#FC4E07")
  ) 
...
r3dmol(
  ...
  # Add model to scene
  m_add_model(data = prova, format = "pdb") %>% 
...
data_4_pdb <- raw_data %>%
  ...
  dplyr::right_join(raw_data, by = "id") %>%
  dplyr::mutate(
